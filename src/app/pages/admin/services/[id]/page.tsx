...
const AdminSelector: React.FC<AdminSelectorProps> = ({
  ...
  useEffect(() => {
    const fetchAdmins = async () => {
      try {
        const res = await api.get("/adminpanel/get-admins/");
        setAdmins(res.data);
      } catch (err) {
    ...
    return (
      <Select disabled>
        <SelectTrigger className="w-full">
          <SelectValue placeholder="Loading admins..." />
        </SelectTrigger>
      </Select>
    );
  }
...
const AdminServiceEdit: React.FC = () => {
  ...
  const fetchService = async (): Promise<void> => {
    try {
      ...
      setValue("admin", serviceData.admin?.id || null);
      setSelectedAdmin(serviceData.admin?.id || null);
  ...
  // Handle admin change
  const handleAdminChange = (adminId: number): void => {
  ...
  const onSubmit = async (data: ServiceFormData): Promise<void> => {
    ...
    // Validate admin
    if (!data.admin) {
      data.admin = user ? user.id : null;
      if (!data.admin) {
        toast.error("Please select an admin for this service");
        return;
      }
    }
    setLoading(true);
    try {
      ...
      formData.append("admin_id", data.admin.toString());
  ...
  const handleCancel = (): void => {
    if (isEditMode) {
      ...
      if (service) {
        ...
        setValue("admin", service.admin?.id || null);
        setSelectedAdmin(service.admin?.id || null);
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-4xl">
      <div className="flex justify-between items-center mb-8">
        ...
        {!isEditMode && (
          <Button onClick={() => setIsEditMode(true)}>Edit Service</Button>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {isEditMode
                ? "Update the basic details about your service."
            ...
            <div>
              ...
              {isEditMode ? (
                ...
                <div className="mt-1 p-2 bg-gray-50 rounded text-sm">
                  {service.admin
                    ? `${service.admin.first_name} ${service.admin.last_name} (${service.admin.email})`
                    : "No admin assigned"}
                </div>
              )}
              {errors.admin && (
                ...
                {plans.map((plan, index) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        {isEditMode ? (
                          <Input
                            ...
                            value={plan.plan}
                            onChange={(e) =>
                              updatePlan(index, "plan", e.target.value)
                            }
                      ...
                      <div>
                        ...
                        {isEditMode ? (
                          <Input
                            ...
                            value={plan.cost}
                            onChange={(e) =>
                              updatePlan(index, "cost", e.target.value)
                            }
                            placeholder="0.00"
                          />
                        ) : (
                      ...
                      <div>
                        <Label htmlFor={`plan-${index}-discount`}>
                          Discount ($)
                        </Label>
                        {isEditMode ? (
                          <Input
                            ...
                            value={plan.discount}
                            onChange={(e) =>
                              updatePlan(index, "discount", e.target.value)
                            }
                            placeholder="0.00"
                          />
                        ) : (
                      ...
                      <div className="md:col-span-2">
                        <Label htmlFor={`plan-${index}-description`}>
                          Description
                        </Label>
                        {isEditMode ? (
                          <Input
